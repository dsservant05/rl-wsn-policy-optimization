...
def immediate_average_reward_func(s,z,action,Swind,lmbda):
   
    if action == -1:
        return (s - z)**2
    elif action!=-1:
        return (lmbda*((Swind[s]-Swind[action])**2)) + ((1-lmbda)*((Swind[s] - Swind[z])**2))

def greedy_policy(s,z,b,p,Swind,lmbda,eta):

    min = np.inf
    argmin = 0
    for a in action_space_func(b,p,eta,Swind):
        temp = immediate_average_reward_func(s,z,a,Swind,lmbda)
        if temp < min:
            min = temp
            argmin = a
    return argmin   
...
def policy_evaluation(Swind, P, lmbda, B, eta, Delta, alpha, tau, gamma, beta, theta, Kmin):
    ...
    while D > theta or iteration <= Kmin:
        for s,z,b,p in product(Ss,Sz,Sb,Sp):
            a = greedy_policy(s,z,b,p,Swind,lmbda,eta)
            # since action is deterministic we dont need to sum over probabilities of every possible action given policy
            avg_q = q_func(Swind, P, lmbda, B, eta, Delta, alpha, tau, gamma, beta, V, s, z, b, p, a)
            V_new[s,z,b,p] = avg_q 
